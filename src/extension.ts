...
const config: { current: ExtensionConfig } = {
  current: vscode.workspace.getConfiguration(EXTENSION_ID) as ExtensionConfig,
};

function loadConfig() {
  config.current = vscode.workspace.getConfiguration(EXTENSION_ID) as ExtensionConfig;
...
function onChangeTextHandler(event: vscode.TextDocumentChangeEvent | vscode.TextDocument) {
