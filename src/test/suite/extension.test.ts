...
suite('Bicep Test Suite', () => {
	...
	test('Test JSX Attributes', () => {
		...
		assert.strictEqual(test5.parts.length, 4, 'JSX should be split into four parts');
		assert.strictEqual(test5.parts[0].text, '<Route ');
		assert.strictEqual(test5.parts[0].operator, 'path="/" ');
		assert.strictEqual(test5.parts[0].operatorType, 'assignment');
		...
		assert.strictEqual(test5.parts[2].text, '/>} ');
